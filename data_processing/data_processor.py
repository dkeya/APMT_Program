...
class APMTDataProcessor:
    ...
    def _basic_cleanups(self):
        ...
        for col in self.df.columns:
            lower = col.lower()
            if any(k in lower for k in ["phone", "telephone", "household id", "_id", "_uuid"]):
    ...
    def _build_column_mapping(self):
        ...
        gender_col = coalesce_first(self.df, ["Gender", "gender", "GENDER", "Select respondent name"])
    ...
    def _parse_mixed_datetime_series(self, s: pd.Series) -> pd.Series:
        """Match the robust parsing used in utils.data_quality."""
        if s is None:
    ...
    def enhanced_standardize_data_optimized(self):
        """
        Standardization with defensible date parsing (mixed formats).
        """
        try:
            df = self.df.copy()

            # ---- 1. Interview date: choose best available source ----
            date_candidates = [c for c in ["int_date", "_submission_time", "start", "end"] if c in df.columns]
            new_columns = {}
            ...
            for c in date_candidates:
                parsed = self._parse_mixed_datetime_series(df[c])
            ...
            if best_col is not None and best_non_na > 0:
                ...
                new_columns["year"] = int_date.dt.year.astype("Int64")
            else:
                st.warning(
                ...
                new_columns["int_date_std"] = pd.Series(pd.NaT, index=idx, dtype="datetime64[ns]")
                new_columns["month"] = pd.Series(pd.NA, index=idx, dtype="string")
                new_columns["year"] = pd.Series(pd.NA, index=idx, dtype="Int64")
            ...
            if kpmd_col and kpmd_col in df.columns:
                ...
                new_columns["kpmd_registered"] = pd.to_numeric(kpmd_series, errors="coerce").astype("Int64")
            else:
                st.info("ℹ️ KPMD registration field not found. 'kpmd_registered' set to NA for all records.")
                new_columns["kpmd_registered"] = pd.Series([pd.NA] * len(df), index=idx, dtype="Int64")

            # ---- 3. Treatment status flag ----
            ...
            if arm_col and arm_col in df.columns:
                ...
                st.info("ℹ️ Treatment/Control field not found. 'is_treatment' set to NA for all records.")
                new_columns["is_treatment"] = pd.Series([pd.NA] * len(df), index=idx, dtype="Int64")
            ...
            if "month" not in self.df.columns:
                self.df["month"] = pd.Series(pd.NA, index=self.df.index, dtype="string")
            if "kpmd_registered" not in self.df.columns:
                self.df["kpmd_registered"] = pd.Series([pd.NA] * len(self.df), index=self.df.index, dtype="Int64")
            if "is_treatment" not in self.df.columns:
                self.df["is_treatment"] = pd.Series([pd.NA] * len(self.df), index=self.df.index, dtype="Int64")
